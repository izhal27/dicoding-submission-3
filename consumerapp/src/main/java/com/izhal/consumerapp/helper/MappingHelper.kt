package com.izhal.consumerapp.helper

import android.database.Cursor
import com.izhal.consumerapp.db.DatabaseContract
...
object MappingHelper {
  ...
  fun mapCursorToArrayList(cursor: Cursor?): ArrayList<UserDetail> {
    ...
    cursor?.apply {
      while (this.moveToNext()) {
        getUser(cursor)?.let { usersList.add(it) }
      }
    }
  ...
  fun mapCursorToObject(cursor: Cursor?): UserDetail? {
